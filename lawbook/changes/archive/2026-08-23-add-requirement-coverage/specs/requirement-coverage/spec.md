...
and `Covers:` keywords in its body. When `Needs:` is absent, speclaw SHALL apply
the configured `coverage.defaultNeeds` (default `[impl, utest]`). When `Status:`
is absent, speclaw SHALL treat the item as `approved`. speclaw SHALL NOT invent
`Needs:` during adopt.
...
using configured source globs (`impl`, `utest`, `itest` in v1).
