...
form only), and `telemetry`. `init` is excluded because it already opens with the
fuller multi-line brand banner; adding the one-line header would brand it twice.
...
`verify` (exit codes and SARIF/JSON are machine-consumed), `budget --json`,
`doctor --json`, and `coverage` when emitting TAP/JSON or when stdout is not a
TTY; and it SHALL be suppressed for any command whenever standard output is not
a TTY (pipes, redirection, CI).
