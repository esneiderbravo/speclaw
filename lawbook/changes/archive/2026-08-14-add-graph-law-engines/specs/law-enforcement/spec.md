...
verify` CLI twin. Two verification surfaces share one model and one scope matcher
but carry different backend sets by design: the **action-time** surface
(`speclaw_check`, on the keystroke latency budget) evaluates the `path` backend
only; the **batch** surface (`law_verify`) additionally evaluates the
graph-reading `deps` and `graph` backends. The `ast`, `process`, `traceability`,
and `semantic` backends remain declared-only and are defined by later
`executable-laws` slices.
...
line). A law whose `verification.kind` is `deps` or `graph` SHALL additionally
carry a validated rule payload; a law whose `verification` names a backend that
is neither `path` nor evaluated by the batch surface SHALL be written to the
manifest but SHALL NOT be evaluated at runtime.
