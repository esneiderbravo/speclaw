...
### Requirement: Validate and coverage surface EARS and ptest `req~ears-cli-surface~1`

WHEN a user runs `speclaw lawbook validate` or `speclaw coverage`, the system
SHALL report EARS diagnostics and `ptest` coverage defects through those
existing commands. speclaw SHALL NOT add a dedicated top-level MCP tool solely
for listing requirements.

Needs: impl, utest
Status: approved

#### Scenario: Validate prints EARS diagnostics without a new MCP tool
- Given a change with an unstructured requirement under strict severity
- When `speclaw lawbook validate <change>` runs
- Then stdout or the structured result SHALL name the EARS diagnostic
- And the MCP catalog SHALL NOT gain a new requirements-only tool for this change

#### Scenario: Coverage reports missing ptest
- Given an identified requirement with `Needs: ptest` and no `ptest` link
- When `speclaw coverage --json` runs
- Then the report SHALL list `ptest` under uncovered types for that item
...
### Requirement: Index reports cache and skip statistics

`speclaw index` (and `compass_index`) SHALL report separately: files re-extracted,
unchanged by content hash, skipped by stat prefilter, embeddings `computed`,
embeddings `fromCache`, and whether the Merkle root was unchanged. Help text
SHALL document `--force`, `--prune`, and optional cache size / retention flags.

#### Scenario: Help lists force and prune
- Given `speclaw` is installed
- When a user runs `speclaw help` or `speclaw index --help`
- Then the usage text SHALL mention `--force` and `--prune`

#### Scenario: No-op index reports root unchanged
- Given a fully indexed project with no filesystem changes
- When `speclaw index` runs
- Then output SHALL indicate the Merkle root was unchanged (or equivalent)
- And embeddings computed SHALL be 0

### Requirement: Laws compile and import are first-class CLI entries

The CLI SHALL dispatch `speclaw laws compile` to the multidialect law compiler
and `speclaw laws import --from <tool>` to the importer (at least `rulesync`).
Help text SHALL list both. Machine-oriented output MAY use `--json`. These
commands SHALL NOT add new canonical MCP tools; optional exposure via an
existing `speclaw_setup` action is allowed. The branded header MAY appear on
interactive TTY compile output and SHALL NOT appear on `--json` stdout.

#### Scenario: Help lists laws compile and import
- Given `speclaw` is installed
- When a user runs `speclaw help` or `speclaw laws --help`
- Then the usage text SHALL mention `compile`
- And SHALL mention `import`

#### Scenario: Compile JSON has no branded header
- Given an initialised project
- When a user runs `speclaw laws compile --json`
- Then stdout SHALL parse as JSON describing the compile report
- And no branded header SHALL appear on stdout

#### Scenario: Import requires a from tool
- Given an initialised project
- When a user runs `speclaw laws import` without `--from`
- Then the command SHALL exit non-zero with a clear usage error

### Requirement: Hybrid find/search CLI controls

The CLI SHALL expose hybrid retrieval controls on the Compass search/find query
surface: optional `--focus` (paths), `--max-tokens` (token budget), and
`--explain` (per-hit ranking signals). Query stdout SHALL remain free of the
branded header. Help text SHALL document these flags.

#### Scenario: Help lists hybrid controls
- Given `speclaw` is installed
- When a user runs `speclaw help` or the search/find/query help
- Then the usage text SHALL mention `--focus`
- And SHALL mention `--max-tokens` or an equivalent budget flag
- And SHALL mention `--explain`

#### Scenario: Explain output includes ranking signals
- Given an indexed project
- When a user runs search/find with `--explain` and a query
- Then stdout SHALL include per-hit contribution signals (at least ranks or scores used for fusion/rerank)
- And no branded header SHALL appear on stdout

### Requirement: Doctor reports FTS and Node readiness for retrieval

`speclaw doctor` (human and `--json`) SHALL report the running Node version,
whether FTS5 is available in `node:sqlite`, and the active embedder id. When
Node is below the declared `engines.node` floor or FTS5 is unavailable, the
report SHALL warn without failing the process unless `--strict` is set.

#### Scenario: Doctor JSON includes FTS availability
- Given any project directory
- When a user runs `speclaw doctor --json`
...
spec-ownership writer. Help text SHALL list `owners`. `--write` SHALL compile
`team.owners` into `.github/CODEOWNERS`. A non-writing check or diff mode SHALL
be available so CI or humans can detect drift without mutating the file. The
branded header MAY appear on interactive TTY human output and SHALL NOT appear
on machine-oriented JSON output when that mode exists. speclaw SHALL NOT add a
dedicated MCP tool solely for writing or mutating CODEOWNERS.

Needs: impl, utest
Status: proposed

#### Scenario: Help lists owners
- Given `speclaw` is installed
- When a user runs `speclaw help`
- Then the usage text SHALL mention `owners`

#### Scenario: Write updates CODEOWNERS without a new MCP tool
- Given a project with a valid `team.owners` map
- When a user runs `speclaw owners --write`
- Then `.github/CODEOWNERS` SHALL contain an updated speclaw owners block
- And the MCP catalog SHALL NOT gain a dedicated owners-mutating tool

#### Scenario: Check mode reports drift without writing
- Given `.github/CODEOWNERS` whose speclaw block does not match `team.owners`
- When a user runs the owners check/diff mode (without `--write`)
- Then the process SHALL exit non-zero or otherwise signal drift
- And the CODEOWNERS file content SHALL be unchanged
