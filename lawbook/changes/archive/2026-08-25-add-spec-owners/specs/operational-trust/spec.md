...
### Requirement: Doctor reports ceremony level distribution

`speclaw doctor` SHALL include a check summarising the distribution of
confirmed ceremony levels among archived changes (counts per level 0–3, and
how many lack `change.json` and are treated as level 3). Invalid `ceremony`
threshold configuration SHALL warn and name that defaults are in use. The
distribution check is informational: a high share of level 0 SHALL NOT alone
be status `error`.

#### Scenario: Doctor JSON includes level counts
- Given at least one archived change with `change.json` recording a level
- When `speclaw doctor --json` runs
- Then a ceremony-level distribution check SHALL be present
- And it SHALL expose per-level counts

#### Scenario: Invalid ceremony cuts warn
- Given `lawbook/config.yaml` with non-increasing ceremony cut values
- When `speclaw doctor --json` runs
- Then a ceremony-config check SHALL have status `warn` or `error`
- And its remedy SHALL mention fixing the cuts or accepting defaults

### Requirement: Doctor reports change type distribution

`speclaw doctor` SHALL include a check summarising the distribution of change
types among archived changes (counts for `feature`, `bug`, and changes without
`changeType` treated as feature). The check is informational: a high share of
bug changes SHALL NOT alone be status `error`.

#### Scenario: Doctor JSON includes change type counts
- Given at least one archived change with `changeType: "bug"` in its metadata
- When `speclaw doctor --json` runs
- Then a change-type distribution check SHALL be present
- And it SHALL expose feature and bug counts

### Requirement: Doctor reports MCP tool surface cost

`speclaw doctor` SHALL report the measured token cost of registered MCP tool
definitions for the active exposure profile, the canonical tool count, and
whether deprecated aliases were invoked recently. When personalized agent files
reference retired tool names, doctor SHALL list them with their replacements.

#### Scenario: Doctor JSON includes surface cost
- Given a scaffolded project
- When `speclaw doctor --json` runs
- Then a tool-surface check SHALL include canonical tool count and measured
  definition-token cost

#### Scenario: Deprecated alias usage is visible
- Given a deprecated MCP alias was invoked since install
- When `speclaw doctor --json` runs
...
`team.owners`, the system SHALL report whether the speclaw owners block is
present, whether any non-empty content appears after `# <<< speclaw:owners`
(status `error` when it does, because last-match would be overridden), and
whether declared owner tokens fail local syntax checks. WHEN a managed owners
block is present, doctor SHALL warn that CODEOWNERS is decorative unless
branch protection requires review from Code Owners, without requiring network
access to confirm that setting. IF `team.owners` is absent, THEN these checks
SHALL be skipped or reported as `skip`.

Needs: impl, utest
Status: proposed

#### Scenario: Content after end marker is an error
- Given a CODEOWNERS file with a user pattern after `# <<< speclaw:owners`
- And `team.owners` is declared
- When `speclaw doctor --json` runs
- Then an owners-related check SHALL have status `error`
- And its detail or remedy SHALL mention that the last matching pattern wins

#### Scenario: Decorative CODEOWNERS warning
- Given a managed speclaw owners block is present
- When `speclaw doctor --json` runs offline
- Then a check SHALL warn that CODEOWNERS without *Require review from Code
  Owners* is decorative
- And the process SHALL NOT require a GitHub API call for that warning

#### Scenario: Absent team.owners skips owners checks
- Given no `team.owners` declaration
- When `speclaw doctor --json` runs
- Then owners-block positioning SHALL NOT be reported as `error` solely because
  no speclaw owners block exists
