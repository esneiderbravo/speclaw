...
Delta spec for the reporting, verification-safety, and archive-gate behavior of
the workflow. This supersedes the canonical `lawbook-workflow` spec, keeping the
sync reconciliation, discipline-report structure, and archive gate unchanged and
adding a data-safety rule to the build step's verification.
