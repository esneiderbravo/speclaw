...
moves, and branch switches do not recompute vectors.
...
Compass schema version SHALL be `"9"`. Opening a schema `"8"` database SHALL
migrate in place (embeddings preserved). Opening other stale databases SHALL
...
- When Compass opens the database under schema `"9"`
...
Under schema `"9"`, every indexed definition node SHALL have a `node_metrics`
...
default includes. `compass_find` SHALL merge lexical and semantic search behind
`mode: exact | concept`. `compass_diff_context` SHALL return changed symbols,
aggregated blast radius, affected tests, and hotspots for a git revision or
working tree in one call. Retired names (`compass_search`, `compass_recall`,
`compass_impact`, `compass_trace`, `compass_affected_tests`, `compass_hotspots`,
`compass_coupling`, `compass_watch`) SHALL delegate through deprecation aliases
...
#### Scenario: Find serves exact and concept modes
- Given an indexed project
- When `compass_find` is invoked with `mode: exact` and a symbol name
- Then results SHALL come from structural name search
- When `compass_find` is invoked with `mode: concept` and a prose query
- Then results SHALL come from embedding recall
