...
the CI checks, the context-budget ceiling, and the merge protection that
together stop unverified or non-compliant code from reaching `main`. Governs
`docs/standards/testing-standards.md`, `.github/workflows/ci.yml`,
`token-budget.json`, and the branch-protection configuration.
...
### Requirement: Declared context budget is a suite gate

`npm test` SHALL include a budget gate that measures speclaw's always-on context
cost with the project's deterministic estimator and FAILS when the measured
total exceeds the total declared in committed `token-budget.json`, or when any
per-surface ceiling in that file is exceeded. The gate SHALL NOT perform network
I/O. The failure message SHALL include the per-surface breakdown.

#### Scenario: Context budget overrun fails the suite
- Given measured always-on context cost above the declared total
- When `npm test` runs
- Then the command exits non-zero
- And the failure output SHALL include the per-surface token table

#### Scenario: Budget gate stays offline
- Given the budget gate test
- When it runs
- Then it SHALL NOT open a network connection
