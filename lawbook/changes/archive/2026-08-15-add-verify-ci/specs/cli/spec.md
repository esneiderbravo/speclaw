...
query family (`explore`, `search`, `recall`, `impact`, `trace`), `mcp`, and
`verify` (exit codes and SARIF/JSON are machine-consumed); and it SHALL be
suppressed for any command whenever standard output is not a TTY
