...
"Shallow-clone detection"). It SHALL be fail-soft. The engine SHALL ALSO expose
richer per-file activity (at least commits, lines changed, and distinct authors)
suitable for hotspot ranking, either by extending `churn` with an explicit opt-in
or via a sibling function, without breaking existing callers that only need
commit counts. Consumers that do not pass a `since` for hotspot ranking SHALL be
able to use a 90-day default at the Compass layer.
...
#### Scenario: Richer activity includes authors and lines
- Given two commits by different authors that each add lines to `src/a.ts`
- When richer file activity is requested for the repo
- Then `src/a.ts` SHALL report at least two commits, positive lines changed, and
  at least two distinct authors
...
SHALL carry a `shallow` marker. It SHALL be fail-soft. The engine SHALL support
excluding commits that touch more than `maxFilesPerCommit` files (default 50
when used by Compass coupling) and SHALL be able to report Jaccard-style
`strength` for a seeded path (or for pairs) as
`both / (commits(A) + commits(B) - both)` without requiring consumers to
re-parse git output.
...
#### Scenario: Oversized commits are excluded from coupling
- Given a commit touching 80 files and no other co-commits for a pair among them
- When co-change analysis runs with `maxFilesPerCommit` equal to 50
- Then that commit SHALL NOT contribute to any pair's `both` count
...
This layer SHALL NOT register MCP tools or CLI commands of its own; it is
consumed only in-process by other modules (e.g. hotspots/coupling under
`code-graph` / `cli`).

#### Scenario: Engine stays transport-free
- Given the change is applied
- When the git-history module is inspected
- Then it SHALL NOT define an MCP tool or CLI verb; hotspots and coupling live
  in Compass/CLI surfaces instead
