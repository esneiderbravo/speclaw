...
`doctor --json`, `coverage` when emitting TAP/JSON or when stdout is not a
TTY, and `drift --json` or when drift stdout is not a TTY; and it SHALL be
suppressed for any command whenever standard output is not a TTY (pipes,
redirection, CI).
